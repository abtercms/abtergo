...
func init() {
	...
	err := v.RegisterValidation("not_before_date", validation.ValidateNotBeforeDate)
	if err != nil {
		panic(fmt.Errorf("failed to register 'not before date' validator. err: %w", err))
	}
	err = v.RegisterValidation("etag", validation.ValidateEtag)
	if err != nil {
...
type Entity struct {
	...
	DeletedAt *time.Time `json:"deleted_at,omitempty" validate:"gtecsfield=UpdatedAt" fake:"{daterange2:[2025-01-01],[2025-12-31]}"`
}
...
func (e Entity) WithID() EntityInterface {
	if e.ID != "" {
		return e
	}
