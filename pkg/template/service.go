...
func (s *service) Create(ctx context.Context, entity Template) (Template, error) {
	if entity.ID != "" {
		return Template{}, arr.New(arr.InvalidUserInput, "payload must not include an id. id in payload: '%s'", entity.ID)
	}

	if err := entity.Validate(); err != nil {
		return Template{}, arr.Wrap(arr.InvalidUserInput, err)
	}
...
func (s *service) Update(ctx context.Context, id string, entity Template, etag string) (Template, error) {
	if entity.ID != "" && entity.ID != id {
		return Template{}, arr.New(arr.InvalidUserInput, "path and payload ids do not match. id in path: '%s', id in payload: '%s'", id, entity.ID)
	}

	if err := entity.Validate(); err != nil {
		return Template{}, fmt.Errorf("payload validation failed. err: %w", arr.Wrap(arr.InvalidUserInput, err))
	}
